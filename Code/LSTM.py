...
def lstm_model(input_size=1, hidden_size=32, num_layers=2):
    """
    定义LSTMStockPredictor模型
    :param input_size: 输入数据的特征数量，默认为1
    :param hidden_size: LSTM隐藏层神经元数量，默认为32
    :param num_layers: LSTM的层数，默认为2
    :return: 实例化后的LSTMStockPredictor模型对象
    """
    class LSTMStockPredictor(nn.Module):
        ...
        def forward(self, x):
            ...
            out = self.fc(out[:, -1, :])
            return out
    return LSTMStockPredictor(input_size, hidden_size, num_layers)


def train_model(model, X_train, y_train, epochs, lr):
    """
    训练模型
    :param model: 要训练的模型对象（LSTMStockPredictor实例）
...
def evaluate_model(model, X_test, y_test, scaler):
    ...
    :param model: 训练好的模型对象（LSTMStockPredictor实例）
...
if __name__ == "__main__":
    ...
    # 构造训练集和测试集的输入输出序列（假设以过去多少天预测下一天价格）
    sequence_length = 150
